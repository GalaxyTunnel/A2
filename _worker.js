...
// ============================================
// GLOBAL PROXYIP POOL — Multi-Region for Worldwide Connectivity
// ============================================
...
var dohURLs = [
    ...
    // Quad9 (Global, privacy-focused)
    "https://dns.quad9.net/dns-query",
    "https://9.9.9.9/dns-query",
    // AliDNS (China-optimized)
    "https://dns.alidns.com/dns-query",
    // DNSPod (China-optimized)
...
// ============================================
// HEALTH CHECK & PROXYIP MANAGEMENT
// ============================================
let proxyIPIndex = 0;
let healthyProxyIPs = [];
let lastHealthCheck = 0;
const HEALTH_CHECK_INTERVAL_MS = 60000; // 60 seconds
const HEALTH_CHECK_TIMEOUT_MS = 3000;     // 3 seconds per check
...
// ============================================
// PROXYIP MANAGEMENT — Global Rotation + Health Check
// ============================================

async function fetchExternalProxyList(rawUrl) {
    if (!rawUrl || rawUrl.includes("YOUR_USERNAME") || rawUrl.includes("example.com")) {
        return [];
    }
    try {
        ...
        console.error("External ProxyIP Fetch Error:", err.message);
    }
    return [];
}

async function healthCheckProxy(hostname, port = 443) {
    try {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), HEALTH_CHECK_TIMEOUT_MS);
        const socket = connect({ hostname, port });
        const writer = socket.writable.getWriter();
        await writer.write(new Uint8Array([0x16, 0x03, 0x01, 0x00, 0x05, 0x01, 0x00, 0x02, 0x00, 0x05]));
        writer.releaseLock();
...
async function refreshHealthyProxies(externalUrl) {
    const now = Date.now();
    if (now - lastHealthCheck < HEALTH_CHECK_INTERVAL_MS && healthyProxyIPs.length > 0) {
        return;
    }

    let candidates = [...GLOBAL_PROXY_POOL];

    // Add external proxies if available
    const external = await fetchExternalProxyList(externalUrl);
    ...
    // Health check in parallel (batch of 5)
    const healthy = [];
    for (let i = 0; i < candidates.length; i += 5) {
        const batch = candidates.slice(i, i + 5);
        const results = await Promise.all(
            batch.map(async (host) => {
                const ok = await healthCheckProxy(host, 443);
                return ok ? host : null;
            })
        );
        healthy.push(...results.filter(Boolean));
    }

    // Fallback: if none healthy, use all candidates (blind trust)
    healthyProxyIPs = healthy.length > 0 ? healthy : candidates;
    lastHealthCheck = now;
    console.log(`Health check complete. ${healthyProxyIPs.length}/${candidates.length} proxies healthy.`);
}

function getNextProxyIP() {
    if (healthyProxyIPs.length === 0) {
        return GLOBAL_PROXY_POOL[Math.floor(Math.random() * GLOBAL_PROXY_POOL.length)];
    }
    const ip = healthyProxyIPs[proxyIPIndex % healthyProxyIPs.length];
    proxyIPIndex++;
    return ip;
}
...
<p style="margin-top:10px;color:#94a3b8;font-size:14px;">Worker is running in global mode with ${GLOBAL_PROXY_POOL.length} built-in proxy endpoints.</p>
</div>
</body></html>`,
        ...
        // Pre-warm healthy proxy list in background
        ctx.waitUntil(refreshHealthyProxies(githubProxyURL));
...
// ============================================
// WebSocket Handler — Dual Protocol (Global)
// ============================================
...
// ============================================
// TCP Outbound — Global ProxyIP with Retry
// ============================================
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, responseHeader, log) {
    ...
    async function retry() {
        const activeProxy = getNextProxyIP();
        ...
        try {
            ...
            log(`Retry failed for ${target}: ${err.message}`);
            // Try one more proxy
            const fallbackProxy = getNextProxyIP();
            if (fallbackProxy !== target) {
                log(`Trying fallback proxy: ${fallbackProxy}`);
                try {
                    const tcpSocket2 = await connectAndWrite(fallbackProxy, portRemote);
                    remoteSocketToWS(tcpSocket2, webSocket, null, log);
                } catch (err2) {
                    log(`Fallback also failed: ${err2.message}`);
                    safeCloseWebSocket(webSocket);
                }
            } else {
                safeCloseWebSocket(webSocket);
    ...
    try {
        ...
        log(`Direct connection failed: ${err.message}, trying proxy...`);
        retry();
    }
}
...
function processVlessHeader(vlessBuffer, userID2) {
    ...
    try {
        ...
        return { hasError: true, message: "Invalid VLESS UUID format" };
    }
...
<head>
  ...
  <title>Galaxy-Tunnel VLESS / TROJAN — Global</title>
  <style>
...
<body>
  ...
  <div class="card-frame">
    ...
    <div class="content-bottom">
      ...
      <div class="subtitle">VLESS / TROJAN — GLOBAL</div>
